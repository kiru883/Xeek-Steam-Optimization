...
if __name__ == '__main__':
    ...
    BAD_FEATURES = [
        'FT_DIST_PAT_3', 'Lin_Dist_Prod_Factor', 'groups_num', 'steam_inj_nums'
    ]

    needest_columns = [i for i in needest_columns if i not in BAD_FEATURES]

    # test model
    model = RandomForestRegressor(n_estimators=n_trees, random_state=random_state, criterion=criterion, n_jobs=-1, max_features=0.1)


    scores, _ = get_cv_rmse_score_1(model, preproc_df,
